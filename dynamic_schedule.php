                            ...
                            <table id="example1" style="align-items: stretch;"
                                ...
                                <tbody>
                                    ...
                                    while ($fetch = $row->fetch_assoc()) {
                                        $id = $fetch['id']; ?><tr>
                                        <td><?php echo ++$sn; ?></td>
                                        <td><?php echo getTrainName($fetch['airline_id']); ?></td>
                                        ...
                                        <td><?php $array = getTotalBookByType($id);
                                                echo (($array['first'] - $array['first_booked'])), " Seat(s) Available for First Class" . "<hr/>" . ($array['second'] - $array['second_booked']) . " Seat(s) Available for Business Class";
                                                ?></td>
                                        ...
                                        <td>
                                            <form method="POST">
                                                ...
                                                <input type="hidden" class="form-control" name="del_airline"
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="" method="post">
                                                        ...
                                                        <p>Train : <select class="form-control" name="airline_id" required
                                                                id="">
                                                                <option value="">Select Airline</option>
                                                                <?php
                                                                    $cons = connect()->query("SELECT * FROM airline");
                                                                    while ($t = $cons->fetch_assoc()) {
                                                                        echo "<option " . ($fetch['airline_id'] == $t['id'] ? 'selected="selected"' : '') . " value='" . $t['id'] . "'>" . $t['name'] . "</option>";
                                                                    }
                                                                    ?>
                                                            </select>
                                                        </p>
                                                        ...
                                                        <p>
                                                           Busniness Class Charge : <input class="form-control"
        ...
        <div class="modal-content" align="center">
            ...
            <div class="modal-body">
                <form action="" method="post">
                    <div class="row">
                        <div class="col-sm-6">
                            Airline : <select class="form-control" name="airline_id" required id="">
                                <option value="">Select Airline</option>
                                <?php
                                $con = connect()->query("SELECT * FROM airline");
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                           ...
                           Business Class Charge : <input class="form-control" type="number" name="second_fee" required
                                id="">
                        </div>
                    </div>
                    <div class="row">
...
if (isset($_POST['submit'])) {
    ...
    $time = $_POST['time'];
    if (!isset($route_id, $airline_id, $first_fee, $second_fee, $date, $time)) {
        alert("Fill Form Properly!");
    } else {
        $conn = connect();
        $ins = $conn->prepare("INSERT INTO `schedule`(`airline_id`, `route_id`, `date`, `time`, `first_fee`, `second_fee`) VALUES (?,?,?,?,?,?)");
        $ins->bind_param("iissii", $airline_id, $route_id, $date, $time, $first_fee, $second_fee);
        $ins->execute();
...
if (isset($_POST['edit'])) {
    ...
    if (!isset($route_id, $airline_id, $first_fee, $second_fee, $date, $time)) {
        ...
        $conn = connect();
        $ins = $conn->prepare("UPDATE `schedule` SET `airline_id`=?,`route_id`=?,`date`=?,`time`=?,`first_fee`=?,`second_fee`=? WHERE id = ?");
        $ins->bind_param("iissiii", $airline_id, $route_id, $date, $time, $first_fee, $second_fee, $id);
        $ins->execute();
...
if (isset($_POST['del_airline'])) {
    $con = connect();
    $conn = $con->query("DELETE FROM schedule WHERE id = '" . $_POST['del_airline'] . "'");
